...
func applyNormalizedSaveToRecord(record saveRecord, normalized saveCreateInput) saveRecord {
	...
	updated.Summary.PortID = strings.TrimSpace(normalized.PortID)
	updated.Summary.PortName = strings.TrimSpace(normalized.PortName)
	updated.Summary.OriginSystemSlug = strings.TrimSpace(normalized.OriginSystemSlug)
	updated.Summary.PortSaveKind = strings.TrimSpace(normalized.PortSaveKind)
	updated.Summary.RelativePath = strings.TrimSpace(normalized.RelativePath)
	updated.Summary.RootRelativePath = strings.TrimSpace(normalized.RootRelativePath)
	updated.Summary.SlotID = strings.TrimSpace(normalized.SlotID)
